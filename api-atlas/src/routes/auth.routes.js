...
r.post("/auth/register", async (req, res) => {
  ...
  res.cookie("token", token, { httpOnly: true, sameSite: "lax", secure: String(process.env.COOKIE_SECURE).toLowerCase()==="true", path:"/" });
...
r.post("/auth/logout", (_req, res) => { res.clearCookie("token", { path:"/" }); res.sendStatus(204); });
